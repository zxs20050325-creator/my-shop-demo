...
function validateInput(input, type = 'string') {
    ...
    if (type === 'string') {
        // 防止路径遍历和其他恶意输入
        return typeof input === 'string' && !input.includes('..') && !input.includes('<script');
    } else if (type === 'number') {
...
app.post('/api/register', (req, res) => {
    ...
    if (!validateInput(username, 'string') || !validateInput(password, 'string')) {
        return res.status(400).json({ success: false, message: '无效的用户名或密码格式' });
    }
...
app.post('/api/cart/add', (req, res) => {
    ...
    // 输入验证
    if (!validateInput(username, 'string') || !validateInput(product, 'object')) {
...
app.post('/api/cart/remove', (req, res) => {
    ...
    if (!validateInput(username, 'string') || !validateInput(index, 'number')) {
        return res.status(400).json({ success: false, message: '无效的输入数据' });
    }
    
    if (userCarts[username] && userCarts[username][index]) {
...
app.post('/api/cart/checkout', (req, res) => {
    ...
    if (!validateInput(username, 'string')) {
        return res.status(400).json({ success: false, message: '无效的用户名' });
    }
    
    const price = parseFloat(totalPrice);
    if (isNaN(price) || price <= 0) {
        return res.status(400).json({ success: false, message: '无效的价格' });
    }
    
    totalRevenue += price;
    totalOrders += 1;
    if (userCarts[username]) userCarts[username] = [];
    addLog(username, '完成支付', `总额 ¥${price}`);
    saveData(); // 新增：保存数据
    res.json({ success: true });
});
...
app.post('/api/favorites/remove', (req, res) => {
    ...
    if (!validateInput(username, 'string') || !validateInput(productId, 'number')) {
        return res.status(400).json({ success: false, message: '无效的输入数据' });
    }
...
function addLog(username, action, product) {
    // 验证输入数据
    if (!validateInput(username, 'string') || !validateInput(action, 'string') || !validateInput(product, 'string')) {
        return; // 如果输入无效，则不记录日志
    }
